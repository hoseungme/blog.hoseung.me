...
현대에는 정말 수많은 종류의 디바이스에서 웹에 접근할 수 있습니다. 또한 각각의 디바이스의 화면 크기는 천차만별입니다.

만약 모든 디바이스에 대해 가로/세로 크기가 큰 이미지 하나만을 제공하고 있다면 어떨까요?

화면 크기가 큰 디바이스에는 적합하겠지만, 화면 크기가 작은 디바이스에는 더 작은 이미지를 사용하는 것이 좋을 것입니다.

또한 디바이스 크기 별로 이미지를 일일히 준비하는 것 보다, 필요에 따라 이미지를 유동적으로 리사이징하여 사용하는 것이 비용이 저렴할 것입니다.

이 글에서는 디바이스의 해상도, 픽셀 밀도 등 배경지식과, 디바이스 크기에 따라 다른 이미지를 제공하는 방법을 알아보고, 간단한 이미지 리사이징 서버도 만들어 보겠습니다.
...
또한 Github의 [custom-device-emulation-chrome](https://github.com/amirshnll/custom-device-emulation-chrome) 레포지토리에 디바이스 별로 해상도와 DPR 정리가 잘 되어있어, 개발자 도구에서 모바일 디바이스를 새로 추가할 때 편리합니다.

# 디바이스 해상도에 맞는 이미지 제공하기

배경지식을 배웠으니 본론으로 돌아옵시다.

`img` element에는 `srcset`과 `sizes`라는 attribute가 있는데요. 이 두 attribute를 잘 명시하여 적절한 크기의 이미지를 제공할 수 있습니다.

`sizes` attribute는 `img` 엘리먼트가 렌더링 될 CSS 상의 크기를 명시합니다. 이 때, 그 크기를 디바이스 크기에 따라 다르게 설정할 수 있습니다.

`srcset` attribute는 렌더링 될 이미지들을 나열합니다. 브라우저는 `sizes` attribute에 명시된 값 또는 디바이스의 DPR 값에 따라 `srcset`에 나열된 이미지들 중 적절한 것을 선택하여 렌더링합니다.

위와 같은 단순 설명으로는 이해하기가 힘든 개념들이라, 예제를 통해서 알아봅시다.

## 디바이스 크기에 따라 다르게 렌더링하기

`srcset`에 이미지들을 나열하는 방식 중 하나는, 이미지의 가로 크기를 명시하는 것입니다. 아래와 같이 `w` descriptor를 사용해 명시할 수 있습니다. 명시한 이미지 각각은 콤마로 구분합니다.
...
왜 디바이스의 viewport width가 300 픽셀인데 선택된 이미지는 `300w.png`가 아니라 `600w.png`일까요?

`w` descriptor로 명시하는 크기는 물리적 픽셀을 의미하기 때문입니다. 예시 디바이스의 DPR이 2이므로, CSS 상의 300 픽셀을 렌더링 하기 위해 물리적 600 픽셀이 필요할 것이고, 따라서 이에 가장 적합한 `600w.png`가 선택된 것입니다.

그럼 `sizes`를 명시하면 어떻게 될까요?
...
예를 들어, DPR이 2인 디바이스의 경우 아래와 같이 `600w.png`가 선택됩니다.

![](./300px-2x-600w-example.png)

이때, DPR이 2인 상황에서, CSS 350 픽셀을 렌더링하기 위해 물리적 600 픽셀은 조금 부족한 상황입니다.

그럼에도 `600w.png`가 선택된 것을 보면, 브라우저가 이미지 선택에 고려하는 것이 화질 말고도 훨씬 많다는 점을 알 수 있습니다. 사용자의 설정, 디바이스의 네트워크 대역폭 등 브라우저 구현에 따라 천차만별일 것 같습니다.

위 예시의 경우엔 Chrome이 100 픽셀의 손실 정도는 `900w.png`를 불러오는 상황을 고려했을 때 감수할 만하다는 결정을 내린 것이겠죠.

이제 한 가지만 더 알고 가면 되는데, `sizes`를 명시할 때는 CSS의 Media Query를 활용하여 브라우저가 여러 가지 사이즈를 조건에 따라 선택하도록 할 수 있습니다. `srcset`에 이미지들을 나열할 때처럼 콤마로 구분하여 나열합니다.
...
`w` descriptor와 `sizes`를 사용하는 것 보다 사용법이 훨씬 간단하니, 본인의 코드베이스에 따라 둘 중 적합한 것을 선택하여 사용하시면 되겠습니다.

## 브라우저 호환성 챙기기

`srcset`이 지원되지 않는 브라우저를 위해 `src`는 항상 기본값으로 넣어주셔야 합니다.
