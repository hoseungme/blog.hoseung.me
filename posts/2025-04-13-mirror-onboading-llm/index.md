...
또한 한국어로 검색했을 때가 영어로 검색했을 때보다 결과 퀄리티가 훨씬 안좋았는데, 왜냐하면 Embedding Vector 변환 모델들의 다국어 처리 성능이 좋지 않았기 때문입니다. 같은 언어로 적힌 텍스트를 Embedding Vector로 변환하여 비교하는 것 보다, 서로 다른 언어로 적힌 텍스트를 Embedding Vector로 변환하여 비교하는 것이 퀄리티가 더 떨어집니다.

이러한 문제를 개선하기 위해 웹사이트 각각을 Embedding Vector로 변환하는 방식을 바꿔봤습니다.

이전에는 웹사이트의 제목과 URL만 Embedding Vector 변환에 사용했다면, LLM을 사용해 아래와 같이 웹사이트 각각에 대한 설명을 만들어 Embedding Vector 변환에 함께 사용했습니다.
...
하지만 이렇게 해도 문제가 전혀 개선되지 않았습니다. 검색 결과의 퀄리티는 개선되지 않았고, 언어 문제는 이런 방법으로 해결할 수도 없었습니다. 게다가 웹사이트 설명을 LLM을 통해 생성했기 때문에, 기껏 RAG를 해놓고선 오히려 위에 설명했던 LLM의 단점을 똑같이 안고가는 꼴이었습니다.

결국 근본적으로 Embedding Vector로 특정 직업과 관련된 웹사이트를 찾아내는 것은 Mirror 온보딩 구현에는 부적합하다는 결론을 내렸습니다.

아예 상황을 바꿔서, 제가 Mirror의 온보딩을 구현하고 있던 것이 아니라 단순히 웹사이트 검색 기능을 구현하고 있던 것이었다고 가정하고, "검색 엔진"에 대한 Vector Search 결과를 봅시다.
...
"검색 엔진"과는 거리가 먼 웹사이트도 보이긴 하지만 그래도 꽤 괜찮은 결과가 나옵니다. 또한, 단순 검색 구현이라고 생각하면 위에서 예시로 보았던 "개발자"에 대한 Vector Search 결과도 그렇게 못봐줄 정도는 아닙니다.

하지만 Mrror 온보딩에 사용하기엔 너무 부족합니다. 검색어가 웹사이트 Vector Search에 사용하기에 비교적 명확한 의미를 가진 "검색 엔진", "노트 정리" 같은 것이라면 괜찮았겠지만, 온보딩에서는 기본적으로 검색어가 "개발자", "디자이너" 같은 직업입니다.

게다가 웹사이트 Vector Search 결과가 온보딩에 사용할 수준이려면, 검색어로 사용한 직업이 무엇인지, 업무시에 어떤 일을 주로 하는지, 성장하기 위해 어떤 지식을 주로 필요로 하는지 등 수많은 문맥이 고려된 결과가 나와야 하는데, Embedding Vector는 당연히 저런 용도로 쓰라고 있는 것이 아닙니다.

물론 Embedding Vector 비교로는 고려될 수 없는 문액을 고려하기 위해 RAG를 하는 것이지만, 그것은 Vector Search 결과를 더욱 좁히기 위한 것이지, 애초에 Vector Search 결과가 빈약한 경우에는 적용하기 어렵습니다.
...
예를 들어, 오늘의 날씨 같은 최신 정보나, 회사 내부의 정보 등, LLM이 학습하지 못한 데이터는 애초에 생성할 수도 없기 때문에 후검증이 불가능하지만, 특정 직업과 관련된 유명한 웹사이트에 대한 정보는 충분히 후검증이 가능합니다.
...
이렇게 LLM으로 웹사이트를 생성한 후 후검증하는 방식에는 어떤 이점이 있을까요?

첫 번째로, 처음에 고려했던 실제 웹사이트 방문 기록을 기반으로 Vector Search를 하는 것 보다 훨씬 정확하고 퀄리티가 좋다는 가장 큰 장점이 있습니다.
...
두 번째로, 후검증에 구글 검색을 사용하기 때문에, 위에서 LLM으로 웹사이트의 설명을 생성했던 것과 다르게 아주 정확한 웹사이트의 제목과 설명을 얻을 수 있습니다. 온보딩을 마치고 세션을 만들 때 웹사이트의 제목이 필요하기 때문에, 이 부분도 아주 중요합니다.

세 번째로, 실제 방문 기록에는 `amazon.de`, `amazon.co.uk` 등 같은 웹사이트에 대해서도 여러 도메인이 포함되어있는 반면, LLM은 거의 `amazon.com`으로만 답변하기 때문에 비교적 일관적인 결과를 만들 수 있습니다.
...
다만 한 가지 문제가 있었는데, Programmable Search Engine을 Custom Search API를 통해 사용하는 경우, 하루에 10000번 밖에 사용하지 못한다는 것이었습니다.

하지만 기존 온보딩에서도 유저가 자신의 직업이 선택지에 없는 경우 "기타"를 선택하고 직업을 직접 입력할 수 있었고, 지난 2년간 유저가 직접 입력한 직업을 모두 DB에 쌓아두었습니다.
...
그리고 주기적으로 유저의 온보딩 이벤트를 읽어서 데이터셋을 업데이트 해주면 온보딩 결과의 퀄리티 유지가 가능하기 때문에 문제가 없다고 판단했습니다.
...
Embedding Vector가 Mirror 온보딩에 사용하기엔 어울리지 않아서 못쓰게 되었지만, 한 가지 확실하게 알게된 점은 있습니다. 더 좋은 Embedding Vector 변환 모델을 사용할 수록 인간의 이해가 들어간 의미 파악을 더욱 잘한다는 것입니다.
