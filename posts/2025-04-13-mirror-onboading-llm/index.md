...
예를 들어, 오늘의 날씨 같은 최신 정보, 회사 내부의 정보 등 LLM이 학습하지 못한 데이터는 애초에 생성할 수도 없기 때문에 당연히 모른다고 하거나 지어낸 정보를 답변할 것이기에 후검증이 무의미하지만, 특정 직업과 관련된 유명한 웹사이트에 대한 답변은 충분히 후검증이 가능합니다.
